...
public enum RegistrationEnvironment {
    ...
    public static boolean shouldRegister(Map<String, Object> annotationData) {
        ...
        if (environment instanceof RegistrationEnvironment registrationEnvironment) {
            return registrationEnvironment.shouldRegister();
        }
        if (environment instanceof String name) {
            return RegistrationEnvironment.valueOf(name).shouldRegister();
        }
        if (environment != null) {
            var text = environment.toString();
            var valueIndex = text.lastIndexOf('.');
            var value = valueIndex >= 0 ? text.substring(valueIndex + 1) : text;
            if (value.endsWith("]")) {
