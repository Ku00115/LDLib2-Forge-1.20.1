...
public class EditorSettings implements IPersistedSerializable {
    ...
    private JsonObject serializeSettings() {
        ...
        for (var entry : settings.entrySet()) {
            ...
            if (codec != null) {
                var data = codec.encodeStart(Platform.createSerializationContext(JsonOps.INSTANCE), settings)
    ...
    private void deserializeSettings(JsonObject json) {
        for (var entry : codecs.entrySet()) {
            ...
            if (json.has(id.toString())) {
                ...
                var result = codec.parse(Platform.createSerializationContext(JsonOps.INSTANCE), data);
                result.result().ifPresent(settings -> this.settings.put(id, settings));
                result.error().ifPresent(e -> LDLib2.LOGGER.error("Failed to load settings for {}, Error {}", id, e));
            }
        }
    }
