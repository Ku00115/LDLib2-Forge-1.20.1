...
//        resourceInstance.addBuiltinProvider(Sprites.getProvider(resourceInstance));
    ...
    public Tag serializeResource(IGuiTexture value, HolderLookup.Provider provider) {
        return IGuiTexture.CODEC.encodeStart(Platform.createSerializationContext(provider, NbtOps.INSTANCE), value).result().orElse(null);
    }

    @Override
    public IGuiTexture deserializeResource(Tag nbt, HolderLookup.Provider provider) {
        return IGuiTexture.CODEC.parse(Platform.createSerializationContext(provider, NbtOps.INSTANCE), nbt).result().orElse(IGuiTexture.MISSING_TEXTURE);
    }
