...
public class UIResource extends Resource<UITemplate> {
    ...
    public Tag serializeResource(UITemplate value, HolderLookup.Provider provider) {
        return UITemplate.CODEC.encodeStart(Platform.createSerializationContext(provider, NbtOps.INSTANCE), value).result().orElse(null);
    }

    @Override
    public UITemplate deserializeResource(Tag nbt, HolderLookup.Provider provider) {
        return UITemplate.CODEC.parse(Platform.createSerializationContext(provider, NbtOps.INSTANCE), nbt).result().orElse(UITemplate.missing());
    }
