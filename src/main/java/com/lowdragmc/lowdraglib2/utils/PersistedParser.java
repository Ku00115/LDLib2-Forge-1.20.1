...
public final class PersistedParser {
    private static CompoundTag wrapTagForStream(Tag tag) {
        var wrapper = new CompoundTag();
        wrapper.put("value", tag);
        return wrapper;
    }

    private static Tag unwrapTagFromStream(CompoundTag wrapper) {
        return wrapper == null ? EndTag.INSTANCE : wrapper.get("value");
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    private static Tag serializeSerializable(INBTSerializable<?> serializable, HolderLookup.Provider provider) {
        if (serializable instanceof IPersistedSerializable persistedSerializable) {
            return persistedSerializable.serializeNBT(provider);
        }
        return serializable.serializeNBT();
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    private static void deserializeSerializable(INBTSerializable serializable, HolderLookup.Provider provider, Tag tag) {
        if (serializable instanceof IPersistedSerializable persistedSerializable && tag instanceof CompoundTag compoundTag) {
            persistedSerializable.deserializeNBT(provider, compoundTag);
            return;
        }
    ...
    public static <T> MapCodec<T> createMapCodec(Supplier<T> creator) {
        return createCodec(creator).fieldOf("data");
    }
    ...
    public static CompoundTag serializeNBT(Object object, HolderLookup.Provider provider) {
        return (CompoundTag) serialize(Platform.createSerializationContext(provider, NbtOps.INSTANCE), object, provider).result().orElse(new CompoundTag());
    }
    ...
    public static void deserializeNBT(CompoundTag tag, Object object, HolderLookup.Provider provider) {
        deserialize(Platform.createSerializationContext(provider, NbtOps.INSTANCE), tag, object, provider);
    }
    ...
    public static void writeBuff(ByteBuf buf, Object object) {
        ...
        var registryBuf = buf instanceof FriendlyByteBuf rb ?
                rb : new FriendlyByteBuf(buf);
                ...
                try {
                    ...
                    if (value != null) {
                        if (value instanceof INBTSerializable<?> serializable) {
                            var tag = serializeSerializable(serializable, provider);
                            ...
                            if (subFlatten) {
                                ...
                                if (mapResult.result().isPresent()) {
                                    mapResult.getOrThrow(false, System.err::println).entries().forEachOrdered(entry ->
                            ...
                            if (!subFlatten) {
                                data = builder.build(op.empty()).getOrThrow(false, System.err::println);
                            }
                        }
                    }
                        ...
                        if (data != null) {
                            if (value instanceof INBTSerializable serializable) {
                                if (op == NbtOps.INSTANCE || op instanceof DelegatingOpsAccessor<?> accessor && accessor.getDelegate() == NbtOps.INSTANCE) {
                                    deserializeSerializable(serializable, provider, (Tag) data);
                                } else {
                                    deserializeSerializable(serializable, provider, op.convertTo(NbtOps.INSTANCE, data));
                                }
                            } else {
                            ...
                            if (value instanceof INBTSerializable serializable) {
                                if (op == NbtOps.INSTANCE || op instanceof DelegatingOpsAccessor<?> accessor && accessor.getDelegate() == NbtOps.INSTANCE) {
                                    deserializeSerializable(serializable, provider, (Tag) op.createMap(map.entries()));
                                } else {
                                    deserializeSerializable(serializable, provider, op.convertTo(NbtOps.INSTANCE, op.createMap(map.entries())));
                                }
                            } else {
    ...
    private static void writeStreamBuffInternal(boolean root, FriendlyByteBuf buf, Class<?> clazz, Object object, HolderLookup.Provider provider) {
            ...
            if (persistent.map(Configurable::subConfigurable, Persisted::subPersisted)) {
                try {
                    ...
                    if (value != null) {
                        ...
                        if (value instanceof INBTSerializable<?> serializable) {
                            var tag = serializeSerializable(serializable, provider);
                            buf.writeNbt(wrapTagForStream(tag));
                        } else {
        ...
        if (root && object instanceof IPersistedSerializable serializable) {
            ...
            if (additional != null && additional != EndTag.INSTANCE) {
                buf.writeBoolean(true);
                buf.writeNbt(wrapTagForStream(additional));
            } else {
                buf.writeBoolean(false);
            }
            serializable.afterSerialize();
            ...
            if (persistent.map(Configurable::subConfigurable, Persisted::subPersisted)) {
                try {
                    ...
                    if (buf.readBoolean() && value != null) {
                        if (value instanceof INBTSerializable serializable) {
                            var tag = unwrapTagFromStream(buf.readNbt(NbtAccounter.UNLIMITED));
                            if (tag != null) {
                                deserializeSerializable(serializable, provider, tag);
                            }
                        } else {
