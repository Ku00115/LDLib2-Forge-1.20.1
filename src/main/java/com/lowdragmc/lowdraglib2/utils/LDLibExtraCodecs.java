...
public final class LDLibExtraCodecs {
    ...
    public static final Codec<org.joml.Vector4f> VECTOR4F = Codec.FLOAT
            .listOf()
            .comapFlatMap(
                    list -> Util.fixedSize(list, 4).map(l -> new org.joml.Vector4f(l.get(0), l.get(1), l.get(2), l.get(3))),
    ...
    public static final Codec<ItemStack> ITEM_STACK = new Codec<>() {
        ...
        public <T> DataResult<Pair<ItemStack, T>> decode(DynamicOps<T> ops, T input) {
            ...
            var realOp = Platform.createSerializationContext(Platform.getClientRegistryAccess(), ops);
            result = ItemStack.CODEC.decode(realOp, input);
            if (result.result().isPresent()) return result;

            realOp = Platform.createSerializationContext(Platform.getServerRegistryAccess(), ops);
