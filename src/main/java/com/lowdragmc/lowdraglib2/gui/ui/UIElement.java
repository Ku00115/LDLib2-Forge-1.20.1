...
//        }
    ...
    public UIElement copy() {
        return CODEC.encodeStart(Platform.createSerializationContext(NbtOps.INSTANCE), this)
                .result()
                .map(tag -> CODEC.parse(Platform.createSerializationContext(NbtOps.INSTANCE), tag)
    ...
    public void deserializeNBT(HolderLookup.@NotNull Provider provider, CompoundTag tag) {
        ...
        if (tag.contains("children")) {
            ...
            for (var i = 0; i < externalTag.size(); i++) {
                ...
                var result = CODEC.parse(Platform.createSerializationContext(provider, NbtOps.INSTANCE), childTag).result();
                if (result.isEmpty()) {
