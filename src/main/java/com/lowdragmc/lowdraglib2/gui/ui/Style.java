...
public abstract class Style implements IConfigurable, IPersistedSerializable {
    ...
    public CompoundTag serializeNBT(HolderLookup.@NotNull Provider provider) {
        ...
        for (Property<?> property : getProperties()) {
            ...
            if (inline != null) {
                tag.put(property.name, property.codec.encodeStart(Platform.createSerializationContext(provider, NbtOps.INSTANCE), cast(inline)).result().orElseThrow());
            }
        }
        return tag;
    }
    ...
    public void deserializeNBT(HolderLookup.@NotNull Provider provider, @NotNull CompoundTag tag) {
        ...
        for (Property<?> property : getProperties()) {
            ...
            property.codec.parse(Platform.createSerializationContext(provider, NbtOps.INSTANCE), tag.get(property.name)).result()
