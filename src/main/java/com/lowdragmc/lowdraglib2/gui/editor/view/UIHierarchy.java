...
public class UIHierarchy extends UIElement {
    ...
    public void copySelected() {
        ...
        var tags = nodes.stream()
                ...
                .map(node -> CODEC.encodeStart(Platform.createSerializationContext(NbtOps.INSTANCE), node.key)
                        .result().orElse(null))
    ...
    public void pasteToSelected() {
        ...
        if (ClipboardManager.INSTANCE.paste() instanceof NodeCopy nodeCopy) {
            List<CompoundTag> copiedNodes = nodeCopy.copiedNodes();
            copiedNodes.forEach(tag -> {
                CODEC.parse(Platform.createSerializationContext(NbtOps.INSTANCE), tag).result().ifPresent(element -> {
