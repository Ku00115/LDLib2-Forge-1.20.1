...
public interface ISyncMangedHolder extends IManagedHolder, IAsyncLogic {
    ...
    Object createSyncPacket(BitSet changed, byte[] data, CompoundTag extra);
        ...
        if (rootStorage.hasDirtySyncFields()) {
            ...
            try {
                server.executeIfPossible(() -> {
                    ...
                    LDLNetworking.sendToTrackingChunk(packet, serverLevel, this.getTrackingPos());
                });
    ...
    default CompoundTag serializeInitialData(HolderLookup.Provider provider) {
        ...
        var ctx = Platform.createSerializationContext(provider, NbtOps.INSTANCE);
