...
public class CollectionAccessor<TYPE> implements
    ...
    public <T> void writeReadOnlyValue(DynamicOps<T> op, Collection<TYPE> value, T payload) {
        var stream = op.getStream(payload).getOrThrow(false, System.err::println);
        if (childAccessor instanceof IDirectAccessor<TYPE> directAccessor) {
            value.clear();
    ...
    public Object[] obtainManagedMark(@NotNull Collection<TYPE> value) {
        return value.stream().map(v -> {
            ...
            if (childAccessor instanceof IDirectAccessor<TYPE> directAccessor) {
                return directAccessor.readDirectVar(Platform.createSerializationContext(NbtOps.INSTANCE), ManagedHolderVar.of(v));
            }
            if (childAccessor instanceof IReadOnlyAccessor<TYPE> readOnlyAccessor) {
                return readOnlyAccessor.readReadOnlyValue(Platform.createSerializationContext(NbtOps.INSTANCE), v);
            }
            throw new IllegalArgumentException("Child accessor %s is not managed for collection accessor".formatted(childAccessor));
        }).toArray();
    }
    ...
    public boolean areDifferent(Object[] managedMark, @NotNull Collection<TYPE> value) {
        ...
        for (var mark : managedMark) {
            ...
            if (childAccessor instanceof IMarkFunction markFunction) {
                ...
                if (!directAccessor.readDirectVar(Platform.createSerializationContext(NbtOps.INSTANCE), ManagedHolderVar.of(v)).equals(mark)) {
                    return true;
                }
            } else if (childAccessor instanceof IReadOnlyAccessor<TYPE> readOnlyAccessor) {
                if (!readOnlyAccessor.readReadOnlyValue(Platform.createSerializationContext(NbtOps.INSTANCE), v).equals(mark)) {
                    return true;
                }
            } else {
                throw new IllegalArgumentException("Child accessor %s is not managed for collection accessor".formatted(childAccessor));
