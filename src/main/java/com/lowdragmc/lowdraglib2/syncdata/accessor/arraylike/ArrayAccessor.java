...
public class ArrayAccessor<TYPE, TYPE_ARRAY> implements IArrayLikeAccessor<TYPE, TYPE_ARRAY> {
    ...
    private <T> T readListField(DynamicOps<T> op, IRef<TYPE>[] refs) {
        ...
        return listBuilder.build(op.empty()).getOrThrow(false, System.err::println);
    }
    ...
    public <T> void writeField(DynamicOps<T> op, IRef<TYPE_ARRAY> ref, T payload) {
        ...
        if (isReadOnly()) {
            ...
            writeListField(op, ref, op.getStream(payload).getOrThrow(false, System.err::println).toList(), refs);
        } else {
            var refs = arrayRef.getRefs();
            ...
            var payloads = op.getStream(payload).getOrThrow(false, System.err::println).toList();
