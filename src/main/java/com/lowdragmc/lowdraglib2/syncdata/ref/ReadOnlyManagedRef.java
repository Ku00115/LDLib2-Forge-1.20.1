    ...
    public final void readSyncToStream(FriendlyByteBuf buffer) {
        if (isReadOnlyManaged()) {
            ...
            if (value == null) {
                ...
                tag.put("value", getReadOnlyVar().getManagedVar().serializeUid(value));
                buffer.writeNbt(tag);
    ...
    public final void writeSyncFromStream(FriendlyByteBuf buffer) {
        if (isReadOnlyManaged()) {
            ...
            if (buffer.readBoolean()) {
                ...
                var uidTag = buffer.readNbt(NbtAccounter.UNLIMITED);
                var uid = uidTag == null ? null : uidTag.get("value");
                var value = readRaw();
                var managedVar = field.getManagedVar();
    ...
    public final <T> T readInitialSync(DynamicOps<T> op) {
        if (isReadOnlyManaged()) {
            ...
            return op.mapBuilder()
                    ...
                    .build(op.empty()).getOrThrow(false, System.err::println);
        } else {
