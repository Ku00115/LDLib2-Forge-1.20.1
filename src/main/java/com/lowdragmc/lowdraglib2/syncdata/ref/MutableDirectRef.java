 ...
 * If the {@link IMarkFunction} is not implemented, it will use codec to store the mark in a type of {@link net.minecraft.nbt.NbtOps}
 */
@Getter
...
public final class MutableDirectRef<TYPE> extends DirectRef<TYPE> {
    ...
    public MutableDirectRef(IVar<TYPE> field, ManagedKey key, IDirectAccessor<TYPE> accessor) {
        ...
        oldValueMark = value == null ? null :
                ...
                markFunction.obtainManagedMark(getField().value()) :
                        accessor.readDirectVar(Platform.createSerializationContext(NbtOps.INSTANCE), field);
    }
    ...
    protected void updateSync() {
        ...
        if (accessor instanceof IMarkFunction markFunction) {
            ...
            if (oldValueMark == null) {
                oldValueMark = accessor.readDirectVar(Platform.createSerializationContext(NbtOps.INSTANCE), field);
                markAsDirty();
            } else {
                var newValueMark = accessor.readDirectVar(Platform.createSerializationContext(NbtOps.INSTANCE), field);
