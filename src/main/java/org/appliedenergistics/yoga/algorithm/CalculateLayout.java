...
package org.appliedenergistics.yoga.algorithm;

import org.appliedenergistics.yoga.YogaAlign;
import org.appliedenergistics.yoga.YogaConstants;
import org.appliedenergistics.yoga.YogaDimension;
import org.appliedenergistics.yoga.YogaDirection;
import org.appliedenergistics.yoga.YogaDisplay;
import org.appliedenergistics.yoga.YogaErrata;
import org.appliedenergistics.yoga.YogaExperimentalFeature;
import org.appliedenergistics.yoga.YogaFlexDirection;
import org.appliedenergistics.yoga.YogaJustify;
import org.appliedenergistics.yoga.YogaNode;
import org.appliedenergistics.yoga.YogaOverflow;
import org.appliedenergistics.yoga.YogaPhysicalEdge;
import org.appliedenergistics.yoga.YogaPositionType;
import org.appliedenergistics.yoga.YogaSize;
import org.appliedenergistics.yoga.YogaWrap;
import org.appliedenergistics.yoga.event.LayoutData;
import org.appliedenergistics.yoga.event.LayoutPassReason;
import org.appliedenergistics.yoga.event.LayoutType;
import org.appliedenergistics.yoga.event.YogaEvent;
import org.appliedenergistics.yoga.event.YogaEventType;
import org.appliedenergistics.yoga.node.CachedMeasurement;
import org.appliedenergistics.yoga.node.LayoutResults;
import org.appliedenergistics.yoga.node.LayoutableChildren;
import org.appliedenergistics.yoga.numeric.Comparison;
import org.appliedenergistics.yoga.numeric.FloatOptional;

import java.util.Iterator;
import java.util.concurrent.atomic.AtomicInteger;

import static org.appliedenergistics.yoga.algorithm.AbsoluteLayout.layoutAbsoluteDescendants;
import static org.appliedenergistics.yoga.algorithm.AlignUtil.fallbackAlignment;
import static org.appliedenergistics.yoga.algorithm.AlignUtil.resolveChildAlignment;
import static org.appliedenergistics.yoga.algorithm.Baseline.calculateBaseline;
import static org.appliedenergistics.yoga.algorithm.Baseline.isBaselineLayout;
import static org.appliedenergistics.yoga.algorithm.BoundAxis.boundAxis;
import static org.appliedenergistics.yoga.algorithm.BoundAxis.boundAxisWithinMinAndMax;
import static org.appliedenergistics.yoga.algorithm.BoundAxis.paddingAndBorderForAxis;
import static org.appliedenergistics.yoga.algorithm.Cache.canUseCachedMeasurement;
import static org.appliedenergistics.yoga.algorithm.FlexDirectionUtil.dimension;
import static org.appliedenergistics.yoga.algorithm.FlexDirectionUtil.flexStartEdge;
import static org.appliedenergistics.yoga.algorithm.FlexLine.calculateFlexLine;
import static org.appliedenergistics.yoga.algorithm.PixelGrid.roundLayoutResultsToPixelGrid;
import static org.appliedenergistics.yoga.algorithm.SizingMode.measureMode;
import static org.appliedenergistics.yoga.algorithm.TrailingPosition.needsTrailingPosition;
import static org.appliedenergistics.yoga.algorithm.TrailingPosition.setChildTrailingPosition;
...
public class CalculateLayout {
    ...
    public static void calculateLayout(
        ...
        if (node.hasDefiniteLength(YogaDimension.WIDTH, ownerWidth)) {
            ...
            width = ownerWidth;
            widthSizingMode = Comparison.isUndefined(width) ? SizingMode.MAX_CONTENT
                    : SizingMode.STRETCH_FIT;
        }
        ...
        if (node.hasDefiniteLength(YogaDimension.HEIGHT, ownerHeight)) {
            ...
            height = ownerHeight;
            heightSizingMode = Comparison.isUndefined(height) ? SizingMode.MAX_CONTENT
                    : SizingMode.STRETCH_FIT;
        }
    ...
    private static void calculateLayoutImpl(
        ...
        assert Comparison.isUndefined(availableWidth) ?
                widthSizingMode == SizingMode.MAX_CONTENT : true :
        ...
        assert Comparison.isUndefined(availableHeight) ?
                heightSizingMode == SizingMode.MAX_CONTENT : true :
        ...
        final float paddingAndBorderAxisMain =
                BoundAxis.paddingAndBorderForAxis(node, mainAxis, direction, ownerWidth);
        ...
        final boolean mainAxisOverflows =
                (sizingModeMainDim != SizingMode.MAX_CONTENT) &&
                        totalMainDim > availableInnerMainDim;

        if (isNodeFlexWrap && mainAxisOverflows &&
                sizingModeMainDim == SizingMode.FIT_CONTENT) {
        ...
        for (; startOfLineIterator.hasNext(); lineCount++) {
            FlexLine flexLine = calculateFlexLine(
                    node,
                    ownerDirection,
                    ownerWidth,
                    mainAxisOwnerSize,
            ...
            if (sizingModeCrossDim == SizingMode.MAX_CONTENT ||
                    sizingModeCrossDim == SizingMode.FIT_CONTENT) {
                ...
                for (YogaNode child : flexLine.getItemsInFlow()) {
                    ...
                    if (alignItem == YogaAlign.STRETCH &&
                        ...
                        if (!child.hasDefiniteLength(
                            ...
                            final SizingMode childWidthSizingMode =
                                    Comparison.isUndefined(childWidth) ||
                                            (!isMainAxisRow && crossAxisDoesNotGrow)
                                            ? SizingMode.MAX_CONTENT
                                            : SizingMode.STRETCH_FIT;
                            final SizingMode childHeightSizingMode =
        ...
        if (performLayout && (isNodeFlexWrap || isBaselineLayout(node))) {
            ...
            final float unclampedCrossDim = sizingModeCrossDim == SizingMode.STRETCH_FIT
        ...
        if (sizingModeMainDim == SizingMode.MAX_CONTENT ||
    ...
    private static void computeFlexBasisForChild(
        ...
        if (resolvedFlexBasis.isDefined() && Comparison.isDefined(mainAxisSize)) {
            ...
            childWidthSizingMode = SizingMode.MAX_CONTENT;
            childHeightSizingMode = SizingMode.MAX_CONTENT;
            ...
            if (childStyle.getAspectRatio().isDefined()) {
                if (!isMainAxisRow && childWidthSizingMode == SizingMode.STRETCH_FIT) {
                    ...
                    childHeightSizingMode = SizingMode.STRETCH_FIT;
                        ...
                        isMainAxisRow && childHeightSizingMode == SizingMode.STRETCH_FIT) {
            ...
            final boolean hasExactWidth =
                    Comparison.isDefined(width) && widthMode == SizingMode.STRETCH_FIT;
            ...
            final boolean hasExactHeight =
                    Comparison.isDefined(height) && heightMode == SizingMode.STRETCH_FIT;
    ...
    private static void measureNodeWithMeasureFunc(
        ...
        if (widthSizingMode == SizingMode.STRETCH_FIT &&
                heightSizingMode == SizingMode.STRETCH_FIT) {
            ...
            node.setLayoutMeasuredDimension(
                    boundAxis(
                            ...
                            YogaFlexDirection.ROW,
                            direction,
                            (widthSizingMode == SizingMode.MAX_CONTENT ||
                                    widthSizingMode == SizingMode.FIT_CONTENT)
            ...
            node.setLayoutMeasuredDimension(
                    boundAxis(
                            ...
                            YogaFlexDirection.COLUMN,
                            direction,
                            (heightSizingMode == SizingMode.MAX_CONTENT ||
                                    heightSizingMode == SizingMode.FIT_CONTENT)
    ...
    private static boolean measureNodeWithFixedSize(
        ...
        if ((Comparison.isDefined(availableWidth) &&
                widthSizingMode == SizingMode.FIT_CONTENT && availableWidth <= 0.0f) ||
                (Comparison.isDefined(availableHeight) &&
                        heightSizingMode == SizingMode.FIT_CONTENT && availableHeight <= 0.0f) ||
                (widthSizingMode == SizingMode.STRETCH_FIT &&
                        heightSizingMode == SizingMode.STRETCH_FIT)) {
            node.setLayoutMeasuredDimension(
                    boundAxis(
                ...
                childCrossSize += marginCross;
                    ...
                    !Float.isNaN(availableInnerCrossDim) &&
                            !currentLineChild.hasDefiniteLength(
                                    FlexDirectionUtil.dimension(crossAxis), availableInnerCrossDim) &&
                            sizingModeCrossDim == SizingMode.STRETCH_FIT &&
    ...
    private static void justifyMainAxis(
        ...
        if (sizingModeMainDim == SizingMode.FIT_CONTENT &&
                flexLine.getLayout().getRemainingFreeSpace() > 0) {
